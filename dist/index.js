 ...
 */
function buildPluginConfigEntries(config) {
    const byId = new Map();
    // Merge an entry into the map; later calls override earlier config keys.
    const upsert = (pluginId, enabled, cfg) => {
    ...
    const prune = (o) => Object.fromEntries(Object.entries(o).filter(([, v]) => v !== undefined && v !== ''));
    const ttsCfg = prune({
        ...
        try {
            ...
            if (err instanceof EnrollmentError) {
                ...
                if (err.code === 'enrollment_disabled') {
                    throw new Error('Hub refused open enrollment: this Hub does not allow secret-less ' +
                ...
                if (err.code === 'unauthorized') {
                    throw new Error('Hub rejected the bootstrap secret. Verify the secret in the ' +
                        'plugin config matches what the Hub super-admin configured for ' +
                        'HUB_BOOTSTRAP_SECRET_HASH.');
                }
                if (err.code === 'not_found') {
                    throw new Error('Hub returned 404 on /bridge/enroll — enrollment is not enabled on ' +
    ...
    if (pluginConfigs.length > 0) {
        ...
        bridge.onWelcome(() => {
            bridge
                ...
                const notRecognisedByHub = pushedIds.filter((id) => !appliedIds.has(id) && !erroredIds.has(id));
                if (errors.length === 0 && notRecognisedByHub.length === 0) {
                    logger.info('pushed plugin config to Hub', { applied: ack.applied });
                    return;
                }
                logger.error('plugin config push had failures — some settings did NOT apply on the Hub', {
                    applied: ack.applied ?? [],
                    errors,
                    notRecognisedByHub,
                    hint: 'If an error mentions auth/decrypt/authenticate, the Hub master key likely ' +
 ...
 */
function buildPublishToHubTool(bridge) {
    ...
    return {
        ...
        description: 'Mirror a locally-created meeting up to the Hub. Required between ' +
        ...
        async handler(input) {
            ...
            return {
                ...
                meetingId: input.meetingId,
                mirroredAt: Date.now(),
                note: 'bridge.meeting.opened sent (fire-and-forget). Hub upsert is ' +
 ...
 */
const MEETING_PLAYBOOK = {
    pacing: 'PACING A MULTI-AGENT MEETING (when text-to-voice narration is ON):\n' +
    ...
    presentation: 'LIVE PRESENTATIONS (PDF / PPTX):\n' +
...
function buildMeetingPlaybookTool() {
    ...
    return {
        ...
        toolset: 'swarm_admin.meeting',
        description: 'Load the current best-practice playbook for running meetings on the Meeting Hub — how to ' +
        ...
        async handler(input) {
            ...
            const sections = topic === 'all' ? topics : topics.filter((t) => t === topic);
            const playbook = sections.map((t) => MEETING_PLAYBOOK[t]).join('\n\n');
            return { ok: true, topic, topics, playbook };
        },
    };
}
function buildAwaitNarrationTool(bridge) {
    const Input = z.object({
        meetingId: z.string().min(1),
        turnId: z
            .string()
    ...
    return {
        ...
        toolset: 'swarm_admin.meeting',
        description: "Wait until a meeting turn's spoken narration (text-to-voice) has finished playing for " +
        ...
        emoji: '⏳',
        async handler(input) {
            const res = await bridge.waitForNarration(input.meetingId, input.turnId, input.timeoutMs ?? 45_000);
            return { ok: true, ...res };
        },
    };
}
function buildPresentOpenTool(bridge) {
    ...
    return {
        ...
        description: 'Drive a LIVE, page-synchronised presentation for the whole room — operator AND Hub guests ' +
 ...
 */
function buildListPluginsTool(bridge) {
    const Input = z.object({
    ...
    return {
        ...
        toolset: 'swarm_admin.meeting',
        description: 'List Meeting Hub plugins available to this tenant, with mime/extension ' +
        ...
        async handler(input) {
            ...
            if (input.meetingId !== undefined)
                frame['meetingId'] = input.meetingId;
            const reply = await bridge.request(frame, 'bridge.plugin.list.result');
            return {
                ok: true,
 ...
 */
function buildSetPluginConfigTool(bridge) {
    const Input = z.object({
        configs: z
            .array(z.object({
    ...
    return {
        ...
        toolset: 'swarm_admin.meeting',
        description: 'Configure Meeting Hub plugins for this tenant: push provider API keys, ' +
 ...
 */
function subscribeBridgeFrame(bridge, frameType, listener) {
    const wrapped = (frame) => {
...
function buildHubBridgeService(bridge) {
    return {
        ...
        onGuestLeft(listener) {
            return subscribeBridgeFrame(bridge, 'bridge.guest.left', (raw) => {
                const f = raw;
                if (typeof f.meetingId !== 'string' || typeof f.peerId !== 'string')
                    return;
        ...
        setPluginConfig: (configs) => bridge.setPluginConfig(configs),
        ...
        uploadArtefact: (input) => bridge.uploadArtefact({
            meetingId: input.meetingId,
            artefactId: input.artefactId,
            bytes: input.bytes,
            mime: input.mime,
